...
import { ValidationResponse } from "../shared/response/types";
...
/**
 * Validate General Journal prior to Posting.
 */
export async function validateGeneralJournal(
    journalId: string
): Promise<ValidationResponse> {

    const res = await fetch(`${BASE_URL}/${journalId}/validate`, {
      credentials: "include",
    });

    if (!res.ok) {
      const message = await res.text();
      throw new Error(`Failed to validate journal:${message}`);
    } 
...
export async function postGeneralJournal(
  journalId: string,
  versionID: number
): Promise<ValidationResponse<GeneralJournal>> {
    const res = await fetch(`${BASE_URL}/${journalId}/post`, {
		method: "PATCH",
		credentials: "include",
		headers: {
  	...
  	// if not an ok response
	if (!res.ok) {
		const { detail } = await res.json();
		throw new Error(`Failed to validate journal: ${detail}`);
	}

	return res.json() as Promise<ValidationResponse<GeneralJournal>>;
}
